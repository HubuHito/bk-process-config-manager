<template>
  <header class="header">
    ...
    <div class="header-nav" v-test.common="'headNav'">
      ...
      <router-link class="nav-item" to="/process-manage">{{
        $t('进程管理')
      }}</router-link>
      <router-link class="nav-item" to="/process-attr">{{
        $t('进程属性')
      }}</router-link>
      <router-link class="nav-item" to="/config-file">{{
        $t('配置文件')
      }}</router-link>
      <router-link class="nav-item" to="/task-history">{{
        $t('任务历史')
      }}</router-link>
      <router-link v-if="showStaticRouter" class="nav-item" to="/statistics">{{
        $t('运营统计')
      }}</router-link>
    </div>
  ...
  methods: {
    ...
    async getBizList() {
      try {
        ...
        const { data: bizList } = await this.$store.dispatch(
          'cmdb/ajaxGetBizList'
        )
        if (bizList.length) {
          bizList.forEach((item) => {
            ...
            item.view_business = item.permission
              ? !!item.permission.view_business
              : false
          })
          this.bizList = bizList.sort(
            (current, next) =>
              Number(next.view_business) - Number(current.view_business)
          )
          this.updateBizList(bizList)
          const targetBiz =
            this.$route.query.biz || localStorage.getItem('bizId')
          if (
            targetBiz &&
            bizList.some((item) => item.bk_biz_id === targetBiz)
          ) {
    ...
    async resetAuthInfo() {
      const currentBiz = this.bizList.find(
        (item) => item.bk_biz_id === this.bizId
      )
