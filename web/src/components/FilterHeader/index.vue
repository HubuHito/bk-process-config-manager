<template>
  <section>
    <div
      class="filter-header"
      :class="{ default: filterList.length === 0 }"
      @click.stop="handleShow($event)"
    >
      {{ name }}
      <i
    ...
    <div v-show="false">
      <div class="label-menu-wrapper" ref="labelMenu">
        ...
        <div
          ...
          v-if="isMultipleChoice && filterList.length"
        >
          <bk-button text @click="handleConfirm" ext-cls="footer-btn mr20">{{
            $t('确定')
          }}</bk-button>
          <bk-button text @click="handleResetSelected" ext-cls="footer-btn">{{
            $t('重置')
          }}</bk-button>
        </div>
      </div>
...
export default {
  ...
  computed: {
    ...
    currentList() {
      return this.isEmpty(this.searchValue)
        ? this.list
        : this.list.filter(
            (item) =>
              item.name &&
  ...
  methods: {
    ...
    handleConfirm() {
      ...
      if (this.selectIds.length) {
        $emit(this, 'confirm', this.property, this.list)
      } else {
