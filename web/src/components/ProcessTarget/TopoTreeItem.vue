  ...
  <ul class="topo-tree">
    ...
    >
      ...
      >
        ...
        <div :class="['node-flex-start', { expanded: node.topoExpand }]">
          <template v-if="node.ableExpand">
            <svg
              v-if="node.topoLoading && loadPosition === 'left'"
              class="svg-icon"
              aria-hidden="true"
            >
        ...
        <!-- 节点类型 -->
        <span
          v-if="nodeTypeMap[node.topoType]"
          :class="`word-icon ${node.topoType}`"
        >
        ...
        <span v-bk-overflow-tips class="node-name" v-else>{{
          node.topoName
        }}</span>
        <div v-if="node.topoTotalCount !== undefined" class="node-total-box">
          {{ node.topoTotalCount }}
        </div>
      </div>
      <TopoTreeItem
      ...
      >
        ...
        <span
          v-else-if="!node.topoLoading && node.empty"
          class="node-child-empty"
        >
      ...
      <div
        v-else-if="node.topoExpand && node.topoloaded && node.empty"
