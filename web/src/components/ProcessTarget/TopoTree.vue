...
export default {
  ...
  watch: {
    data(value) {
      ...
      $emit(this, 'format-tree', this.nodeList)
    },
  },
  methods: {
    handleClick(node) {
    ...
    handleToggleCheck(node) {
      ...
      $emit(this, 'toggle-check', node)
    },
    handleToggleExpand(node) {
      node.topoExpand = !node.topoExpand
      $emit(this, 'toggle-expand', node)
    },
    ...
    formatTopoData(data, topoLevel = -1, parent = null) {
      ...
      data.reduce((nodeList, item) => {
        const nodeItem = {
          ...
          topoReduceName:
            parent && parent.id
              ...
              : item.topoName,
          topoProcess:
            parent && ['serviceInstance', 'instance'].includes(parent.topoType), // 可优化 this.transNodeValue(item, this.checkType)
          topoChecked: false,
          // topoTotalCount: 888
        }
        ...
        nodeItem.topoConcatPath =
          parent && parent.uuid
            ? `${parent.topoConcatPath} / ${nodeItem.topoName}`
            : nodeItem.topoName
    ...
    transNodeValue(node, condition) {
      return condition && condition.length
        ? condition.includes(node.topoType)
        : true
    },
