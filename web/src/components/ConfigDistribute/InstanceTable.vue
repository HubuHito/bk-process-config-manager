<template>
  <div class="instance-table-container">
    <div class="header">
      <div class="header-left">
        <div
          ...
          :class="!showTable && 'close'"
          @click="showTable = !showTable"
        ></div>
        <div class="config-name">
          {{ configTemplate.template_name + ' / ' + configTemplate.file_name }}
        </div>
        <i18n
      ...
      <div v-if="curStep === 1" class="header-right-1">
        ...
        <template v-else-if="selectedVersionIds.length">
          <div class="version-selector selected" @click="toggleVersionList">
            ...
            {{
              $t('已选x个版本', {
                x: `${selectedVersionIds.length}/${usedVersionList.length}`,
              })
            }}
          </div>
        ...
        <template v-else>
          <template v-if="usedVersionList.length">
            <div
              ...
              v-bk-tooltips="$t('高级选项，可按版本进一步过滤下发对象')"
      ...
      <div v-if="curStep === 2" class="header-right-2">
        ...
        <div
          class="statue-counter-tips"
          v-show="statusCounter.pending + statusCounter.running"
        >
          <span class="num loading">{{
            statusCounter.pending + statusCounter.running
          }}</span>
          {{ $t('生成中') }}
        </div>
      </div>
    ...
    <div
      class="version-list"
      v-show="
        curStep === 1 && showTable && showVersionList && usedVersionList.length
      "
    >
      <bk-checkbox-group
        class="king-checkbox-group"
    ...
    <bk-table
      ...
      "
      v-show="showTable"
      v-bkloading="{ loading: tableLoading, zIndex: 0 }"
    >
          ...
          <template #default="{ row }">
            ...
            >
              <bk-button
                ...
                text
                :disabled="
                  row.status === 'not_generated' || !row.config_instance_id
                "
      ...
      <template v-if="curStep === 2">
        ...
        <bk-table-column :label="$t('操作')" min-width="140" key="regenerate">
          <template #default="{ row }">
            ...
            <bk-popover
              :disabled="!!row.config_instance_id"
    ...
    <!-- 查看配置 -->
    <ViewConfig
      :instances-config="viewConfigData"
      @onCloseSide="viewConfigData = null"
    ></ViewConfig>
  </div>
</template>
...
export default {
  ...
  emits: ['update-version', 'update:isTasking'],
  data() {
    return {
      versionList: [], // 版本列表
