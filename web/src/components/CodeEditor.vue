...
export default {
  ...
  mounted() {
    this.initEditor()
    this.setEditorBackground()
    $on(window.bus, 'resize', this.layoutEditor)
    $on(window.bus, 'navigation-toggle', this.onNavigationToggle)
    window.addEventListener('resize', this.layoutEditor, { passive: true })
  },
  beforeUnmount() {
    $off(window.bus, 'resize', this.layoutEditor)
    $off(window.bus, 'navigation-toggle', this.onNavigationToggle)
  ...
  methods: {
    async initEditor() {
      ...
      $emit(this, 'change', this.getContent())
    ...
    setEditorBackground() {
      if (this.readonly) {
        this.$refs.codeEditorRef.querySelector(
          '.monaco-editor .margin'
        ).style.backgroundColor = 'rgb(46,46,46)' // 323232
        this.$refs.codeEditorRef.querySelector(
        ...
        this.$refs.codeEditorRef.querySelector(
          '.monaco-editor .margin'
        ).style.backgroundColor = 'rgb(33,33,33)' // 212121
        this.$refs.codeEditorRef.querySelector(
          '.monaco-editor .monaco-editor-background'
        ).style.backgroundColor = 'rgb(29,29,29)' // 1D1D1D
      }
    },
    onNavigationToggle() {
      setTimeout(() => {
