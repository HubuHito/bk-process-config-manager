<template>
  <div class="process-attribute-container">
    ...
    <bk-table
      v-bind="$attrs"
      ref="table"
      v-bkloading="{ loading: loading, zIndex: 0 }"
        ...
        <template v-else>
          <template v-for="head in parent.child">
            ...
            <template v-if="head.type === 'selection'">
              ...
              <bk-table-column v-bind="head" v-else></bk-table-column>
            </template>
            <!-- Column - 自定义slot, slotNameStr -->
            <bk-table-column v-bind="head" v-else-if="head.slotNameStr">
              <template #default="{ row, $index }">
                <slot
                  :name="head.slotNameStr"
                  :row="row"
                  :$index="$index"
                ></slot>
              </template>
              ...
              <template #default="{ row, $index }">
                ...
                >
                  <div class="bk-input-text">
                    <input
                      ...
                      :placeholder="$t('请输入')"
                      @blur.capture="
                        handleEditBlur(...arguments, row[head.prop], head)
                      "
                ...
                >
                  <bk-popover
                    ...
                    theme="light"
                  >
                    <span v-if="!isEmpty(row[head.prop])">{{
                      row[head.prop]
                    }}</span>
                    <div
                      v-else
            ...
            <!-- Column - 常规 -->
            <bk-table-column v-bind="head" v-else>
              <template #default="{ row }">
...
export default {
  ...
  emits: ['rowCheck', 'sub-head-click', 'cell-edit', 'eidt-disabled'],
  data() {
    return {
      editProp: '',
      editIndex: null,
  ...
  methods: {
    ...
    headRenderStatus(h, { column }) {
      ...
      const parentHead = this.tableHead.find(
        (parent) => parent.key === column.property
      )
    ...
    handSubHeadClick({ column, parent }) {
      $emit(this, 'sub-head-click', { column, parent })
    },
    ...
    handleEditBlur(event, cellValue, head) {
      ...
      if (editStr !== cellStr) {
        $emit(this, 'cell-edit', {
          prop: this.editProp,
          value:
            head.type === 'number' ? parseInt(this.editValue) : this.editValue,
    ...
    getGroupStatus(row, child) {
      return child && child.some((key) => !row[key.prop] && row[key.prop] !== 0)
        ? Unfinished
        : Correct
    },
    isEmpty(val) {
