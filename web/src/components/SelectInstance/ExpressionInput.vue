<template>
  ...
  >
    <template v-for="item in fieldsInfo" :key="item.type">
      ...
      <div v-if="item.hasGap" class="gap">.</div>
    </template>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    handleChange(type, e) {
      $emit(this, 'selected', type, e.target.value, this.expressionData)
    },
    ...
    clearSelectedData() {
      ...
      $emit(this, 'selected', null, null, this.expressionData)
    },
    /**
     * 对外暴露的方法，回填表达式
    ...
    setValue(expressionScope, { silent }) {
      ...
      if (!silent) {
        $emit(this, 'selected', 'custom', null, this.expressionData)
      } else {
        $emit(this, 'selected', null, null, this.expressionData)
      }
    },
