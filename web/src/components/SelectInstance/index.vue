<template>
  <div class="select-instance-container" v-test.range="'wrapper'">
    ...
    <div v-bk-tooltips="envTips" class="env-container">
      ...
      >
        <bk-option value="1" :label="$t('测试')"></bk-option>
        <bk-option value="2" :label="$t('体验')"></bk-option>
        <bk-option value="3" :label="$t('正式')"></bk-option>
      </bk-select>
    </div>
    ...
    <ExpressionInput
      v-show="!isDropdownMode"
      ref="expressionSelectorRef"
      @selected="handleExpressionChange"
    />
    <!-- 选择按钮 -->
    <bk-button
      v-test.form="'submit'"
    ...
    <div
      class="button-text"
      :class="{ 'is-hidden': isClearDisabled }"
      v-test.range="'clear'"
      @click="clearSelect"
    >
...
export default {
  ...
  emits: ['update:isExisted', 'init', 'valueChange'],
  data() {
    return {
      inited: false,
      ...
      envTips: {
        content: this.$t(
          '环境类型，对应配置平台中的集群属性。进程筛选范围受限于此属性'
        ),
        delay: 3000,
      },
      fieldsInfo: [
        ...
        {
          type: 'service',
          value: 'bk_service_ids',
          list: 'bk_service_list',
          name: this.$t('服务实例'),
          hasGap: true,
        },
        {
          type: 'processName',
          value: 'bk_process_names',
          list: 'bk_process_name_list',
          name: this.$t('进程别名'),
          hasGap: true,
        },
        {
          type: 'processId',
          value: 'bk_process_ids',
          list: 'bk_process_id_list',
          name: 'process_id',
          hasGap: false,
        },
      ],
      sourceDropdownSelected: {},
      dropdownSelectorData: {
        // 筛选可选项
        bk_set_list: [],
        bk_module_list: [],
        bk_service_list: [],
      ...
      expressionSelectedData: {
        // 记录表达式组件选择的值，计算是否显示清空
        bk_set_name: '*',
        bk_module_name: '*',
  ...
  computed: {
    isClearDisabled() {
      ...
      const isEmptied = this.isDropdownMode
        ? Object.values(this.dropdownSelectedData).every(
            (item) => item.length === 0
          )
        : Object.values(this.expressionSelectedData).every(
            (item) => item === '*'
          )
      $emit(this, 'update:isExisted', !isEmptied)
      return isEmptied
    },
  },
  created() {
    $on(bus, 'clear-select-filter', this.clearSelect)
    this.bizId = this.$store.state.bizId
    this.bizEnvMap = JSON.parse(
      window.localStorage.getItem('BK_SET_ENV_MAP') || '{}'
    )
    ...
    setTimeout(() => {
      ...
      if (!this.inited) {
        const {
          bk_set_ids: sets,
          bk_module_ids: modules,
          bk_service_ids: services,
    ...
    $emit(
      ...
      Object.assign(
        ...
        this.isDropdownMode
          ? { ...this.dropdownSelectedData }
          : { ...this.expressionSelectedData }
      )
    )
  },
  mounted() {
    if (this.isDropdownMode) {
      this.$refs.dropdownSelectorRef.setValue(this.sourceDropdownSelected, {
        silent: true,
      })
    }
  },
  beforeUnmount() {
    $off(bus, 'clear-select-filter', this.clearSelect)
  },
  methods: {
    // 初始化、环境改变时获取所有下拉列表
    ...
    async fetchSetList() {
      try {
        ...
        this.dropdownSelectorData.bk_set_list = Object.freeze(
          res.data.map((item) => ({
            id: item.bk_set_id,
    ...
    async fetchModuleList(bkSetIds = []) {
      ...
      try {
        ...
        this.dropdownSelectorData.bk_module_list = Object.freeze([
          ...noRepeatNameMap.values(),
        ])
    ...
    async fetchServiceList(bkModuleIds = []) {
      ...
      try {
        ...
        const res = await this.$store.dispatch(
          'cmdb/ajaxGetServiceListByModule',
          {
            data: {
        ...
        this.dropdownSelectorData.bk_service_list = Object.freeze(
          res.data.map((item) => ({
    ...
    async fetchProcessList(bkServiceIds = []) {
      ...
      try {
        ...
        const res = await this.$store.dispatch(
          'process/ajaxGetProcessListByService',
          {
            data: {
        ...
        this.dropdownSelectorData.bk_process_name_list = Object.freeze(
          noRepeatNames.map((name) => ({ id: name, name }))
        )
        this.dropdownSelectorData.bk_process_id_list = Object.freeze(
          res.data.map((item) => ({
    ...
    async handleDropdownChange(type, ids, selectedData) {
      ...
      if (type !== null) {
        $emit(this, 'valueChange', this.isDropdownMode, {
          bk_set_env: this.setEnv,
          ...selectedData,
        })
        this.basicLoading = true
    ...
    emitEventWithCurrentValue(eventName) {
      const selectedData = this.isDropdownMode
        ? this.dropdownSelectedData
        : this.expressionSelectedData
      $emit(this, eventName, this.isDropdownMode, {
        bk_set_env: this.setEnv,
        ...selectedData,
      })
    },
    ...
    setScopeValue(scope, options = { silent: false }) {
      ...
      this.$refs.dropdownSelectorRef.setValue(
        this.sourceDropdownSelected,
        options
      )
    },
    /**
          ...
          selectedData[value].reduce((arr, item) => {
            if (typeof item === 'string' && item.indexOf(',') > -1) {
              arr.splice(
                0,
                arr.length,
                ...item.split(',').map((id) => parseInt(id, 10))
              )
            } else {
              arr.push(item)
            }
    ...
    getFormatScope() {
      if (this.isDropdownMode) {
        ...
        fieldsInfo.forEach(({ value, list }) => {
          scope[value] = this.getSelectorDefaultValue(
            dropdownSelectorData[list],
            dropdownSelectedData[value]
          )
        })
        return scope
      }
