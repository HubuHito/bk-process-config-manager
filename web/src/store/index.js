  ...
  mutations: {
    ...
    updateAuthMap(state, authList = []) {
      authList.forEach((auth) => {
        if (auth.action_id) {
          state.authMap[auth.action_id] = !!auth.is_allowed
        }
      })
    },
    ...
    routeConfigTemplateVersionList(state, { templateId }) {
      // 配置文件模板 ==> 版本列表
      router.push(`/config-file/template/${templateId}/version`)
    },
    routeConfigTemplateVersionDetail(
      state,
      { templateId, versionId, isPreview }
    ) {
      // 配置文件模板 ==> 版本详情
    ...
    routeConfigTemplateDistribute(state, { templateId }) {
      // 配置文件模板 ==> 配置下发
      router.push(`/config-file/template/${templateId}/distribute`)
    },
    routeConfigTemplateGenerate(state, { templateId }) {
      // 配置文件模板 ==> 配置生成
      router.push(`/config-file/template/${templateId}/generate`)
    },
    routeConfigTemplateCheck(state, { templateId }) {
      // 配置文件模板 ==> 配置检查
    ...
    routeProcessManageManage() {
      // 进程管理管理详情页
      router.push('/process-manage/manage')
    },
    routeProcessManageConfigInformationInstance(
      state,
      { type, serviceId, processId }
    ) {
      // 进程管理配置进程实例
      if (processId) {
        router.push(
          `/process-manage/manage/${type}/service/${serviceId}/process/${processId}`
        )
      } else {
        router.push(
          `/process-manage/manage/${type}/service/${serviceId}/process`
        )
      }
    },
    routeProcessManageRelease() {
      // 进程管理配置下发页面
      router.push('/process-manage/release-config')
    },
    routeTaskHistoryList(state, highlightIds) {
      // 任务历史列表页
      if (highlightIds) {
    ...
    routeTaskHistoryDetail(state, jobId) {
      // 任务历史列表页
