...
export default {
  ...
  computed: {
    ...
    isDisableGenerateAndPrevious() {
      return (
        this.distributeButtonLoading ||
        this.generateButtonLoading ||
        this.hasRunningTask
      )
    },
    isDisableDistribute() {
  ...
  methods: {
    ...
    async handleSelectInstance(isDropdownMode, selectedScope) {
      try {
        ...
        const res = await this.$store.dispatch(
          ...
          {
            data: {
              [isDropdownMode ? 'scope' : 'expression_scope']:
                this.selectedScope,
    ...
    async handleGenerate() {
      if (
        !this.isGeneratePage &&
        this.curStep === 1 &&
        this.instanceList.every((item) => item.status === 'generated')
      ) {
    ...
    async getTaskList() {
      try {
        ...
        this.instanceList
          .slice(0, this.instanceCounts)
          .forEach((instanceItem, instanceIndex) => {
            ...
            if (taskStatus === 'succeeded') {
              ...
              instanceItem.failed_reason =
                matchTaskItem.extra_data.failed_reason
    ...
    judgeTaskDone() {
      const result =
        this.statusCounter.pending + this.statusCounter.running === 0
    ...
    handlePageAdd(instanceCounts) {
      ...
      if (
        this.jobId !== null &&
        this.isJobReady &&
        !this.isAllTaskSucceededOrFailed
      ) {
        // 需要更新 task_list
        this.getTaskList()
      }
