<template>
  <div class="config-file-template-version-container">
    <router-view
      ...
      v-model:version-loading="versionLoading"
    ...
    selectedVersion() {
      ...
      return (
        this.versionList.find(
          (item) => item.config_version_id === this.$route.params.versionId
        ) || null
      )
    },
  },
        ...
        if (res.data.length) {
          ...
          if (updateVersionId) {
            ...
            if (versionId) {
              // 详情页或预览页
              if (
                !res.data.some((item) => item.config_version_id === versionId)
              ) {
                // 路由请求的版本不存在，默认选中可用或第一项
                const targetVersion =
                  res.data.find((item) => item.is_active) || res.data[0]
                this.$router.replace({
                  params: {
