<template>
  <div class="version-list-container" v-bkloading="{ loading: versionLoading }">
    <div class="template-title" style="padding-bottom: 9px">
      <span
        class="gsekit-icon gsekit-icon-down-line"
        @click="$store.commit('routeConfigTemplateList')"
      ></span>
          ...
          <template v-slot="{ row }">
            <div class="version-description">
              <span
                v-bk-overflow-tips
                :class="{
                  'lighten-text': !row.is_draft && !row.is_active,
                  'text-overflow-row': true,
                }"
              >
                {{ row.description }}
              </span>
          ...
          <template v-slot="{ row }">
            <div class="button-container">
              ...
              <span
                ...
                v-test="'copy'"
                class="button-text"
                @click.stop="handleCopyAndCreate(row)"
              >
                {{ $t('复制并新建') }}
              </span>
              <template v-if="row.is_active">
                <AuthTag
                  ...
                  :class="[
                    'button-text',
                    { 'is-disabled': !authMap.operate_config },
                  ]"
                ...
                <span
                  v-else
                  v-bk-tooltips="$t('未关联进程，无法进行配置下发')"
                  @click.stop
                  class="button-text is-disabled"
                >
                  {{ $t('配置下发') }}
                </span>
              </template>
        ...
        <template v-slot:empty>
          <bk-exception class="empty-box-container" type="empty" scene="part">
            ...
            <template v-else>
              ...
              <div class="button-text" @click="handleCreate">
                {{ $t('新建版本') }}
              </div>
            </template>
          </bk-exception>
