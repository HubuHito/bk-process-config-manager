  ...
  <div class="field-content">
    ...
    <template v-if="isEdit">
      <bk-select
        ...
        :clearable="false"
      >
        <bk-option value="CRLF" label="CRLF - Windows（\r\n）"></bk-option>
        <bk-option value="LF" label="LF - Unix and macOS（\n）"></bk-option>
      </bk-select>
      <bk-input
        v-else
        v-model.trim="initialValue"
    ...
    <template v-else>
      ...
      <span v-else class="field-value" v-bk-overflow-tips>{{
...
export default {
  ...
  methods: {
    ...
    focusOnForm() {
      if (this.field === 'line_separator') {
        // 下拉表单
        const selectTrigger =
          this.$refs.select.$el.querySelector('.bk-select-name')
        selectTrigger.click()
      } else {
    ...
    async handleChange() {
      ...
      try {
        ...
        this.selectedConfig[this.field] = this.initialValue
