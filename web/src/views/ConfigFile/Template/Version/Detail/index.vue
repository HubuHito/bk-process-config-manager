<template>
  <div
    class="version-detail-container"
    v-test="'versionDetail'"
    v-bkloading="{ loading: versionLoading, opacity: 0.1 }"
  >
    <template v-if="selectedVersion">
      <div class="template-title">
        <span
          class="gsekit-icon gsekit-icon-down-line"
          @click="backToVersionList"
        ></span>
      ...
      <div class="detail-main-container">
        ...
        >
          <div class="code-header">
            ...
            <div class="code-header-left">
              <span
                v-bk-overflow-tips="{ distance: 24 }"
                class="id-and-description text-overflow-row"
              >
                #{{ selectedVersion.config_version_id }}
            ...
            <div v-if="selectedVersion.is_draft" class="code-header-right">
              ...
              <bk-select
                ...
                :clearable="false"
              >
                <bk-option value="python" label="python"></bk-option>
                <bk-option value="yaml" label="yaml"></bk-option>
                <bk-option value="json" label="json"></bk-option>
                <bk-option value="javascript" label="javascript"></bk-option>
              </bk-select>
              <span
                v-bk-tooltips="{ content: $t('全屏'), theme: 'black-arrow' }"
                :class="`gsekit-icon gsekit-icon-${
                  isFullScreen
          ...
          <div class="editor-main">
            ...
            <div
              v-if="showMarkersAside"
              class="code-marker-aside"
              :style="{ height: markerAsideHeight + 'px' }"
            >
              <div class="code-marker">
                <ul ref="codeMarkerRef">
                  <li
                    v-for="(marker, index) in currentEditorMarkers"
                    :key="index"
                    class="code-marker-item"
                  >
                    <i class="marker-icon bk-icon icon-close-circle-shape"></i>
                    <span class="marker-message" v-if="marker.type === 'msg'">{{
                      marker.message
                    }}</span>
                    <template v-else>
                      <span class="marker-owner">[{{ marker.owner }}]</span>
                      <span class="marker-message">{{ marker.message }}</span>
                      <span class="marker-code" v-if="marker.code"
                        >[{{ marker.code }}]</span
                      >
              ...
              <DragIcon
                prop="marker"
                placement="top"
                :class="isMarkerDragActive && 'active'"
                @dragBegin="dragBegin"
              />
            </div>
          </div>
        ...
        >
          ...
          <PreviewComponent
            ...
            v-model:preview-content-cache="previewContentCache"
            ref="previewRef"
      ...
      <div v-if="selectedVersion.is_draft" class="code-footer">
        ...
        <bk-button class="king-button dark" @click="backToVersionList">{{
          $t('取消')
        }}</bk-button>
      </div>

      <bk-sideslider
        v-model:is-show="showDiff"
        :quick-close="true"
        :title="$t('版本对比')"
        :width="1200"
      >
...
export default {
  ...
  beforeRouteLeave(from, to, next) {
    // 离开路由
    if (
      this.selectedVersion.is_draft &&
      this.selectedVersion.content !== this.currentEditorContent
    ) {
      // 草稿修改后未保存
      this.$bkInfo({
  ...
  computed: {
    ...
    isDraftUpdated() {
      ...
      return (
        ...
        this.currentEditorContent !== this.previewContentCache
      )
    },
    versionId() {
  ...
  watch: {
    ...
    isFullScreen() {
      $emit(window.bus, 'resize', true)
    },
    showRightPanel() {
      $emit(window.bus, 'resize', true)
    },
    rightPanelWidth() {
      $emit(window.bus, 'resize')
    },
  ...
  methods: {
    ...
    async createNewActiveVersion(description) {
      try {
        this.saveLoading = true
        const res = await this.$store.dispatch(
          'configTemplate/ajaxCreateNewConfigVersion',
          {
        ...
        this.showSaveDialog = false
        this.selectedVersion.is_draft = false // 确保能够离开路由
        this.backToVersionList()
      } catch (e) {
        console.warn(e)
      } finally {
      ...
      if (this.versionList.some((item) => item.is_draft)) {
        ...
        try {
          ...
          $emit(
            this,
            'updateVersionList',
            res.data.config_version_id.toString()
          )
        } catch (e) {
          console.warn(e)
