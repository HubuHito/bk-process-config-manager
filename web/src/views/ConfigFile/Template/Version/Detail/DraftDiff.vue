<template>
  <div class="draft-diff-container">
        ...
        >
          <template #trigger>
            <div class="bk-select-name">
              ...
              >
                #{{ compareVersion.config_version_id }}
          ...
          <template
            v-for="option in filterVersionList"
            :key="option.config_version_id"
          >
            <bk-option
    ...
    <div class="diff-editor-footer">
      <bk-button :loading="isLoading" @click="coverDraftWithSelected">{{
        $t('用对比版本覆盖当前草稿')
      }}</bk-button>
...
export default {
  ...
  watch: {
    selectedVersionId(val) {
      this.compareVersion = this.filterVersionList.find(
