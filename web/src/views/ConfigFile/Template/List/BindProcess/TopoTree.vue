<template>
  <ul class="bind-topo-tree">
    <template v-for="(topoNode, index) in nodeList" :key="index">
          ...
          <template v-else>
            <span
              :class="[
                ...
                topoNode.topoExpand && 'expanded',
              ]"
              :style="{ 'margin-left': 26 * topoNode.topoLevel + 'px' }"
            ></span>

            <!-- 集群 -->
            <span v-if="topoNode.topoType === 'set'" class="word-icon">{{
              $t('集')
            }}</span>
            <!-- 服务模板 -->
            <span
            ...
            <!-- 模块 -->
            <span
              v-else-if="topoNode.topoType === 'module'"
              class="word-icon"
              >{{ $t('模') }}</span
            >
            <!-- 服务实例 -->
            ...
            <span v-bk-overflow-tips class="text-content">{{
              topoNode.topoName
            }}</span>

            <span
              v-if="topoNode.topoProcessCount !== undefined"
              class="process-count"
            >
        ...
        if (topoNode.topoExpand && !(topoNode.child && topoNode.child.length)) {
          try {
            ...
            if (topoNode.topoType === 'module') {
              ...
              const res = await this.$store.dispatch(
                'process/ajaxGetInstanceProcessByServiceInstance',
                {
              ...
              res.data.forEach((item) => {
                ...
                for (let i = 0; i < this.instanceProcess.length; i++) {
                  ...
                  if (
                    selectedItem.__IS_RECOVER &&
                    selectedItem.property.bk_process_id ===
                      item.property.bk_process_id
                  ) {
                ...
                if (findItem) {
                  ...
                  $emit(this, 'update:instanceProcess', copyProcess)
                }
              })
              topoNode.child = res.data
              ...
              const res = await this.$store.dispatch(
                'process/ajaxGetTemplateProcessByServiceTemplate',
                {
                  // topo里面的服务模板使用 service_template_id，直接拉取的服务模板列表使用 id
                  serviceTemplateId:
                    topoNode.service_template_id || topoNode.id,
                }
              )
              res.data.forEach((item) => {
                item.topoParent = topoNode
                ...
                if (findItem) {
                  ...
                  copyProcess.splice(findIndex, 1, item)
                  $emit(this, 'update:templateProcess', copyProcess)
                }
              })
              topoNode.child = res.data
            }
