<template>
  ...
  <div v-else class="process-select-container">
    <div class="bind-process">
      ...
      ></bk-input>
      <div class="scroll-container" v-bkloading="{ loading: isLoading }">
        <TopoTree
          v-show="showTopo"
    ...
    <div class="result-preview">
      ...
      <div class="scroll-container">
        ...
        <div class="process-list-container">
          <div
            class="title"
            @click="showTemplateProcess = !showTemplateProcess"
          >
            <span
              class="bk-icon icon-right-shape"
              :class="showTemplateProcess && 'expanded'"
            ></span>
            {{ $t('已选') }}
          ...
          <ul class="process-list" v-show="showTemplateProcess">
            <template
              v-for="(item, index) in templateProcess"
              :key="index + Date.now()"
            >
              <li class="process-item">
                <div class="white-card">
                  <div v-bk-overflow-tips class="white-card-left">
                    {{ item.topoName }}
                  </div>
                  <div class="white-card-right">
                    <span
                      class="gsekit-icon gsekit-icon-parenet-node-line"
                    ></span>
                ...
                <div class="close-icon-container">
                  <span
                    ...
                    @click="removeTemplateProcess(item, index)"
                  ></span>
                </div>
              </li>
            </template>
        ...
        <div class="process-list-container">
          <div
            class="title"
            @click="showInstanceProcess = !showInstanceProcess"
          >
            <span
              class="bk-icon icon-right-shape"
              :class="showInstanceProcess && 'expanded'"
            ></span>
            {{ $t('已选') }}
          ...
          <ul class="process-list" v-show="showInstanceProcess">
            <template
              v-for="(item, index) in instanceProcess"
              :key="index + Date.now()"
            >
              <li class="process-item">
                <div class="white-card">
                  ...
                  <div
                    class="white-card-right"
                    v-bk-tooltips.bottom="item.topoReduceName"
                  >
                    <span
                      class="gsekit-icon gsekit-icon-parenet-node-line"
                    ></span>
                ...
                <div class="close-icon-container">
                  <span
                    class="bk-icon icon-close"
                    @click="removeInstanceProcess(item, index)"
                  ></span>
                </div>
              </li>
            </template>
  ...
  methods: {
    ...
    filterTopoData(topoData, topoLevel = -1, parent = null) {
      ...
      topoData.forEach((item) => {
        ...
        item.topoReduceName = parent
          ? `${parent.topoReduceName}/${item.topoName}`
          : item.topoName
    ...
    handleCheckNode(topoNode) {
      if (topoNode.topoType === 'templateProcess') {
        if (topoNode.topoChecked) {
          ...
          // 取消选择
          const index = this.templateProcess.findIndex(
            (item) => item === topoNode
          )
        ...
        if (topoNode.topoChecked) {
          ...
          // 取消选择
          const index = this.instanceProcess.findIndex(
            (item) => item === topoNode
          )
