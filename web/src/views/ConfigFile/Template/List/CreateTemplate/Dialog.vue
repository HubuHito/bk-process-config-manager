...
export default {
  ...
  methods: {
    handleValueChange(val) {
      $emit(this, 'update:showCreate', val)
      if (!val) {
        // 关闭时重置数据
    ...
    async handleCreate() {
      try {
        ...
        const res = await this.$store.dispatch(
          'configTemplate/ajaxCreateConfigTemplate',
          {
    ...
    handleCancel() {
      $emit(this, 'update:showCreate', false)
    },
    async handleBind() {
      try {
        ...
        $emit(this, 'created', this.newTemplate.config_template_id)
      } catch (e) {
        console.warn(e)
      } finally {
        this.bindLoading = false
