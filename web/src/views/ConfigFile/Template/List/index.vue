<template>
  <div class="config-file-template-list-container" v-test="'configFile'">
        ...
        <template v-slot="{ row }">
          <div class="template-name-column">
            ...
            <span
              ...
              class="gsekit-icon gsekit-icon-alert"
            ></span>
          </div>
        </template>
      </bk-table-column>
      ...
      <bk-table-column :label="$t('关联进程数')">
        <template v-slot="{ row }">
          <bk-popover v-if="row.is_bound" placement="right">
            <div
              class="button-text"
              style="min-width: 20px; line-height: 28px"
              v-test="'numConnect'"
              @click="operateBind(row)"
            >
          ...
          <div
            v-else
            v-bk-tooltips="$t('未关联进程，无法进行配置下发，点击关联')"
            class="not-bound-column"
            v-test="'numConnect'"
            @click="operateBind(row)"
          >
            <span>{{ $t('未关联') }}</span>
              ...
              <template v-slot="{ disabled }">
                ...
                >
                  <bk-button
                    text
                    theme="primary"
                    :disabled="disabled || !row.has_version || !row.is_bound"
                    v-test="'release'"
              ...
              <template v-slot="{ disabled }">
                ...
                >
                  <bk-button
                    text
                    theme="primary"
                    :disabled="disabled || !row.has_release"
                    v-test="'configCheck'"
    ...
    <BindProcessDialog
      ...
      v-model:show-dialog="showBindProcess"
...
export default {
  ...
  methods: {
        ...
        confirmFn: async () => {
          try {
            await this.$store.dispatch(
              'configTemplate/ajaxDeleteConfigTemplate',
              {
                templateId: row.config_template_id,
              }
            )
    ...
    handleCreated(templateId) {
      // todo 有点卡 可以看怎么优化下
      $emit(this, 'selectConfig', templateId)
    },
  },
  emits: ['selectConfig'],
}
