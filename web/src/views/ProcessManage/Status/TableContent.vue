<template>
  <div class="bk-custom-table">
    ...
    >
        ...
        <div class="process-status" v-if="row.proc_inst_infos.length">
          ...
          >
            ...
            <div class="item-status">
              {{ $t('状态') + '：' }}
              <StatusView
                v-if="instInfo.process_status === 0"
                type="origin"
                :is-solid="true"
                :text="$t('未运行')"
              />
      ...
      <bk-table-column type="expand"></bk-table-column>
      <bk-table-column
        key="selection"
        width="65"
        :resizable="false"
      ...
      <template v-for="item in setting.selectedFields" :key="item.id">
        ...
        >
          <template #default="{ row }">
            ...
            />
            <StatusView
              v-else-if="row.process_status === 1"
              type="success"
              :is-solid="true"
              :text="$t('运行中')"
            />
            <StatusView
              v-else-if="row.process_status === 2"
              type="failed"
              :is-solid="true"
              :text="$t('未运行')"
            />
            ...
            >
              ...
              <template #content>
                <ul class="dot-menu-list">
                  ...
                  <li class="dot-menu-item" @click="onCheckProcessConfig(row)">
                    {{ $t('查看进程配置') }}
                  </li>
                </ul>
              </template>
  ...
  methods: {
    ...
    getFields() {
      ...
      if (settingCache) {
        ...
        this.setting.selectedFields = this.fields
          .slice(0)
          .filter((m) => defaultColumn.includes(m.id))
    ...
    handleFilterHeaderReset(prop) {
      $emit(this, 'handleFilterHeaderReset', prop)
    },
    // 表头筛选变更
    handleFilterHeaderConfirm(prop, list) {
      $emit(this, 'handleFilterHeaderConfirm', prop, list)
    },
    /**
     * 自定义selection表头
     */
    ...
    async handleCheckAll(value) {
      ...
      $emit(this, 'selectedAllPages', value)
    },
    // 清除选中的所有数据
    onClearSelectedData() {
      this.tableData.forEach((item) => {
    ...
    handlePageChange(page) {
      $emit(this, 'handlePageChange', page)
    },
    handlePageLimitChange(limit) {
      $emit(this, 'handlePageLimitChange', limit)
    },
    // 操作
    operateProcess(type, id) {
      $emit(this, 'operateProcess', type, id)
    },
    // 配置下发
    operateConfigDistribute(row) {
      $emit(this, 'showSideSlider', row)
    },
    // 查看进程配置
    onCheckProcessConfig(row) {
      // , isConfigFile) {
...
.bk-custom-table {
  ...
  .hosting {
    background: rgba(63, 192, 109, 0.2);
    color: #3fc06d;
  }

  .fail {
    background: rgba(239, 149, 153, 0.2);
    color: #ea3636;
  }

  .unmanaged {
    background: rgba(151, 155, 165, 0.2);
    color: #979ba5;
  }
