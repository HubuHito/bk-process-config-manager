<template>
  <div
    ...
    v-test="'processStatus'"
    v-bkloading="{ loading: checkeStatus === 'loading' && isDataLoading }"
  >
    <EmptyProcess
      v-if="showEmptyProcess"
      :completed="checkeStatus === 'completed'"
    ></EmptyProcess>
    <section v-else-if="showNormalProcess">
      ...
      <div v-show="isSelectedAllPages" class="selected-all-pages">
        ...
        {{ $t('条，跨页全选暂不支持反选，') }}
        <span class="button-text" @click="handleClearSelection">{{
          $t('取消')
        }}</span>
      </div>
      <!-- 表格 -->
      <TableContent
        ref="tableContent"
        v-bkloading="{ loading: isDataLoading }"
...
export default {
  ...
  computed: {
    ...
    // 是否允许下发
    isAllowIssued() {
      const result = this.tableData.some(
        (item) =>
          this.processIdList.includes(item.bk_process_id) && item.templateCount
      )
    ...
    isAllowStart() {
      const result = this.tableData.some(
        (item) =>
          this.processIdList.includes(item.bk_process_id) &&
    ...
    tableEmptyType() {
      return this.isExisted || this.searchSelectValue.length
        ? 'search-empty'
        : 'empty'
    },
  },
      ...
      if (val) {
        ...
        if (expressionScope) {
          ...
          if (this.checkedEnv !== '3') {
            this.$nextTick(() => {
              this.$refs.selectInstanceRef.handleSetEnv(
                this.localEnv || this.checkedEnv
              )
            })
          }
  ...
  methods: {
    async getTemplateListSource() {
      ...
      try {
        ...
        const res = await this.$store.dispatch(
          'process/ajaxGetProcessStatusList',
          {
            data: {
    ...
    async getFilterCondition() {
      const res = await this.$store.dispatch(
        'meta/ajaxGetStatusFilterCondition'
      )
      const filterData = []
    ...
    async handleDialogConFirm() {
      try {
        ...
        if (this.isSelectedAllPages) {
          params[this.isDropdownMode ? 'scope' : 'expression_scope'] =
            this.selectedScope
          Object.assign(params, this.getSearchParams())
        } else {
          const copySelected =
            this.$refs.selectInstanceRef.dropdownSelectedData || {}
          const scope = {
    ...
    handlePaste(e) {
      ...
      data.getAsString((value) => {
        ...
        if (isIpType) {
          ...
          const splitCode =
            ['，', ' ', '、', ',', '\n'].find(
              (split) => str.indexOf(split) > 0
            ) || '\n'
          const ipRegx = new RegExp(
    ...
    handlePushValue(prop, values, merged = true) {
      ...
      const index = this.searchSelectValue.findIndex(
        (item) => item.id === prop || item.id === this.toLine(prop)
      )
      if (index > -1) {
        const originValues = merged
          ? this.searchSelectValue[index].values || []
          : []
        values.forEach((value) => {
          const isExist = originValues.some(
            (item) => item && item.id === value.id
          )
          if (!isExist) {
        ...
        const data = this.filterData.find(
          (data) => data.id === prop || data.id === this.toLine(prop)
        )
        this.searchSelectValue.push({
          id: prop,
          name: data ? data.name : '',
    ...
    handleSearchSelectChange(list) {
      ...
      if (list.length > this.prevListLength) {
        ...
        const childrenList = this.filterData.find(
          (item) => item.id === lastValue.id
        )
        if (childrenList && childrenList.children) {
          childrenList.children = childrenList.children.map((child) => {
            const searchValue = lastValue.values.find(
          ...
          if (data.children) {
            data.children = data.children.map((child) => {
              if (!item) {
                ...
                child.checked = item.values.some(
