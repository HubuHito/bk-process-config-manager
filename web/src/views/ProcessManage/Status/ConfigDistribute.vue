    ...
    <template #default>
      <div class="from-area">
          ...
          <div :class="['common-form-item', { 'default-width': !isContrast }]">
            ...
            >
              <div class="generate-btn">
                <i
                  class="swither-icon gsekit-icon gsekit-icon-swither-small"
                ></i>
                <span>{{ $t('生成') }}</span>
              </div>
            </bk-button>
        ...
        <div
          ...
          v-bkloading="{ loading: isDataLoading, opacity: 0.1 }"
        >
          <div class="editor-title">
            <div
              :class="['common-form-editor', { 'default-width': !isContrast }]"
            >
              ...
              <div class="editor-info">
                ...
                <div>
                  ...
                  <template v-if="isContrast">
                    {{
                      releasedConfig.created_at
                        ? modifyFormatDate(releasedConfig.created_at)
                        : '--'
                    }}
                  </template>
                  <template v-else>
                    {{
                      generatedConfig.created_at
                        ? modifyFormatDate(generatedConfig.created_at)
                        : '--'
                    }}
                  </template>
                </div>
        ...
        <div class="king-btn-group" v-if="!isBatch">
          <bk-popover
            :disabled="isDataLoading || Boolean(generatedConfig.created_at)"
            :content="$t('请重新生成')"
          >
            <bk-button
