<template>
  <div
    ...
    v-bkloading="{ loading: !Object.values(jobInfo).length }"
  >
    <section class="detail-config">
      <div class="detail-config-header">
        <i
          class="button-text gsekit-icon gsekit-icon-down-line"
          @click="handleRouterClick"
        ></i>
        <span>{{ $t('任务详情') + $t('：') }}</span>
        <template v-if="Object.values(jobInfo).length">
          <span>{{
            jobObject[jobInfo.job_object] + jobAction[jobInfo.job_action]
          }}</span>
          <span
            :class="[
              'perform-status',
    ...
    <template v-else>
      <section class="detail-search">
        <div class="btn-section">
          <bk-button
            v-test="'retryAll'"
            @click="onAllRetry"
            :disabled="jobInfo.status !== 'failed'"
          ...
          <bk-dropdown-menu
            ...
            font-size="medium"
            :disabled="
              ['running', 'pending'].includes(jobInfo.status) || copyLoading
            "
            @show="dropdownShow"
            @hide="dropdownHide"
          >
      ...
      <section class="tab-header" v-if="statusTabList.length">
        ...
        >
          <span
            v-if="item.err_code === 0"
            class="status-icon generated-status"
          ></span>
          <span
            v-else-if="item.err_code === 1"
            class="status-icon pending-status"
          ></span>
          <span
            v-else-if="item.err_code === 3"
            class="status-icon failed-status"
          ></span>
          <span
            v-else-if="item.err_code === 4"
            class="status-icon ignored-status"
          ></span>
          <svg
            v-else-if="item.err_code === 2"
            class="status-icon running-status"
            aria-hidden="true"
          >
            <use xlink:href="#gsekit-icon-loading"></use>
          </svg>
          <span v-else class="status-icon other-status"></span>
          <span class="tab-item-name" v-bk-overflow-tips>{{
            item.message
          }}</span>
      ...
      <section class="detail-table">
        ...
        >
          <bk-table-column
            prop="set_name"
            :label="() => renderFilterHeader('set_name', $t('集群'))"
          ...
          <bk-table-column
            prop="template_name"
            :label="() => renderFilterHeader('template_name', $t('模块'))"
            show-overflow-tooltip
          >
            <template #default="{ row }">
          ...
          <bk-table-column
            prop="process_name"
            show-overflow-tooltip
            :label="() => renderFilterHeader('process_name', $t('进程别名'))"
          >
            <template #default="{ row }">
    ...
    <ViewConfig :instances-config="instancesConfig" @onCloseSide="onCloseSide">
    </ViewConfig>
    <!-- 配置对比 -->
    <bk-sideslider
      v-model:is-show="sliderData.isShow"
      :quick-close="true"
      :width="1200"
    ...
    >
      ...
      <template>
        <div
          v-bkloading="{ loading: sliderData.isLoading }"
          style="height: calc(100vh - 60px)"
        >
          <SidesliderDiff
            v-if="sliderData.oldData"
...
export default {
  ...
  methods: {
    ...
    async getActiveVersion() {
      try {
        const versionRes = await this.$store.dispatch(
