<template>
  <div class="process-attribute-container" v-bkloading="{ loading: loading }">
    ...
    <div class="page-content" v-show="!loading && !isEmtpyTopo">
      ...
      >
        ...
        <div class="topo-toggle" @click.stop="showLeftTopo = !showLeftTopo">
          <i
            :class="[
              'gsekit-icon gsekit-icon-angle-left-line',
              { 'show-topo': !showLeftTopo },
            ]"
          ></i>
        </div>
      </section>
      ...
      <section class="content-right">
        <EmptyServiceBox
          v-if="isTemplateTab && isEmtpyTemplate"
          type="template"
        ...
        <template v-else>
          <section class="temp-head" v-if="isTemplateTab">
            <p class="title">
              <span class="tag">{{ $t('模版') }}</span
              >{{ selectedTemp.name }}
            </p>
          </section>
          <section class="topo-head" v-else>
            <p class="topo-path text-overflow-row" v-bk-overflow-tips>
              {{ topoProcessTitle }}
            </p>
            <h2 class="topo-name text-overflow-row mt10" v-bk-overflow-tips>
              {{ selectedNode.topoName }}
            </h2>
          </section>
          <div class="head-tips" v-if="isTemplateTab && showSyncTip">
            ...
            <span class="tips-text">{{
              $t(
                '当前服务模板下进程属性已改变，需到配置平台进行同步操作方可生效。'
              )
            }}</span>
            <bk-button class="ml20" text size="small" @click="goSyncTemplate">{{
              $t('去同步')
            }}</bk-button>
          ...
          <section
            ...
            v-bkloading="{ loading: !loading && tableLoading }"
          >
            <ProcessTable
              v-if="!tableLoading"
...
export default {
      ...
      tableHead: [
        ...
        },
        {
          ...
          tips: this.$t('若要使用Gsekit进行进程管理，需完善此基础控制配置', [
            window.PROJECT_CONFIG.APP_NAME,
          ]),
          child: [
            {
              label: this.$t('启动用户'),
        ...
        },
        {
          ...
          child: [
            ...
            {
              label: this.$t('启动等待时长th'),
              prop: 'bk_start_check_secs',
              type: 'number',
              minWidth: 130,
              fixed: false,
            },
  ...
  methods: {
    ...mapActions('cmdb', [
      'ajaxGetProcessRelated',
      'ajaxGetCheckServiceTemplate',
      'ajaxGetBatchCheckServiceTemplate',
    ]),
    ...
    async initTargetInfo() {
      ...
      this.tabActive = ['template', 'node'].includes(this.type)
        ? this.type
        : 'template'
      ...
      if (
        this.isEmtpyTopo ||
        (this.tabActive === 'template' && this.isEmtpyTemplate)
      ) {
        this.loading = false
        return false
      }
      ...
      if (this.tabActive === 'template') {
        ...
        if (node) {
          ...
          if (arr[2]) {
            this.findSelectedTopo(
              { type: 'serviceInstance', id: arr[2] - 0 },
              select,
              this.formatTree
            )
            if (!select.nodeItem) {
        ...
        if (!node || !select.nodeItem) {
          const { bk_obj_id: bkObjId, bk_inst_id: bkInstId } =
            this.formatTree[0]
          this.$store.commit('routeProcessAttr', {
            params: { type: this.tabActive },
      ...
      return new Promise(async (resolve, reject) => {
        try {
          ...
          if (topoRes.data && topoRes.data.length) {
            ...
            this.isEmtpyTopo = !(
              topoRes.data[0].child && topoRes.data[0].child.length
            )
          } else {
          ...
          resolve(
            !this.isEmtpyTopo ||
              (this.type === 'template' && !this.isEmtpyTemplate)
          )
        } catch (e) {
          console.warn(e)
          this.loading = false
    ...
    async loadTableData() {
      ...
      try {
        ...
        const methodName = this.isTemplateTab
          ? 'ajaxGetTemplateProcessByServiceTemplate'
          : 'ajaxGetProcessRelated'
        const res = await this[methodName](
          this.isTemplateTab
    ...
    getUndoneCount() {
      ...
      const hasUndoneRows = this.tableData.filter((row) =>
        undoneKeys.some((key) => row[key] !== 0 && !row[key])
      )
          ...
          this.searchSelectValue.forEach((item) => {
            if (item.id === 'missing') {
              item.values.forEach((option) => {
                const head = this.tableHead.find(
                  (parent) => parent.key === option.id
                )
                if (head) {
                  head.child
              ...
              // 无固定类型的query 放入进程别名、进程名称下过滤
              const rules = ['bk_func_name', 'bk_process_name'].map(
                (field) => ({ field, operator: 'equal', value: item.id })
              )
    ...
    handleTabChange(type) {
      ...
      const query = {}
      if (this.isTemplateTab) {
        this.templateId = this.templateList.length
    ...
    async checkServeTemplate() {
      let showSync = false
      try {
        const hasId = this.isTemplateTab
          ? !!this.templateId
          : this.selectedNode.topoType === 'serviceTemplate'
        const serviceTemplateId = this.isTemplateTab
          ? this.templateId
          : this.selectedNode.service_template_id
    ...
    async handleCellEdit({ row, prop, value }) {
      try {
        ...
        const res = await this.$store.dispatch(
          `process/${
            this.isTemplateTab
              ? 'ajaxUpdateProcessTemplate'
              : 'ajaxUpdateProcessInstance'
          }`,
          {
            data: {
              process_template_id: this.isTemplateTab
    ...
    // 数字、表头筛选
    handleFiltersChange() {
      const undoneFilter = this.searchSelectData.find(
    ...
    handSubHeadFilter({ parent }) {
      this.searchSelectValue.splice(0, this.searchSelectValue.length, {
        ...
        multiable: true,
        name: this.$t('配置缺失'),
        values: [{ id: parent.key, name: parent.label }],
      })
      this.handlePageChange({ page: 1 })
    },
    // 初始化表头
    initTableHead() {
      ...
      this.tableHead.forEach((parent, index) => {
        ...
        if (parent.isExpandable) {
          filterFields = filterFields.concat(
            parent.child.map((item) => item.prop)
          )
        }
        ...
        if (head.isExpandable) {
          ...
          if (length) {
            data.forEach((row) => {
              if (
                head.child.some(
                  (child) => !row[child.prop] && row[child.prop] !== 0
                )
              ) {
                hasUndone = true
              }
    ...
    setTableHead() {
      ...
      let fixedWidth = this.$textTool.getHeadWidth(
        this.tableHead[0].child[0].label
      )
    ...
    goSyncTemplate() {
      ...
      const serviceTemplateId = this.isTemplateTab
        ...
        : this.selectedNode.service_template_id
      window.open(
        `${cmdbUrl}/#/business/${bizId}/service/operational/template/${serviceTemplateId}?tab=instance`
      )
    },
    // 前端筛选 & 分页
    forntfilterData() {
      let data = [...this.sourceTableData]
      const { page, undoneRule, blurryRule, queryRule } =
        this.getParams('front')
      // 且
      if (queryRule && queryRule.length) {
        data = data.filter((item) =>
          queryRule.some((rule) => item[rule.field].includes(rule.value))
        )
      }
      if (blurryRule && blurryRule.length) {
        const filterBlurryRule = blurryRule.filter((rule) => rule.field)
        data = data.filter((item) =>
          filterBlurryRule.every((rule) =>
            item[rule.field].includes(rule.value)
          )
        )
      }
      // 或
      if (undoneRule && undoneRule.length) {
        data = data.filter((item) =>
          undoneRule.some((rule) => !item[rule.field])
        )
      }
      ...
      list.forEach((item) => {
        if (item.computed) {
          item.minWidth = Math.max(
            parent?.minWidth || 0,
            this.$textTool.getHeadWidth(item.label)
          )
        }
...
:deep(.process-edit-slider .bk-sideslider-content) {
  height: 100%;
}
</style>
