<template>
  <section>
    <div class="table-statistics">
      <i18n-t keypath="进程总数" v-if="isBizProcess" class="static-total"
        ><span class="primary">{{ staticTotal }}</span></i18n-t
      >
      <i18n-t keypath="模板进程总数" v-else class="static-total"
        ><span class="primary">{{ staticTotal }}</span></i18n-t
      >
      <span class="static-incomplete" v-if="staticIncomplete">
        {{ $t('配置缺失') }}:
        <span
          class="text-cursor undone"
          v-bk-tooltips="$t('筛选')"
          @click.stop="handleFiltersChange"
        >
          {{ staticIncomplete }}
        </span>
      </span>
    ...
    >
      ...
      <template #alias="{ row }">
        <span
          v-if="row.showTypeTag"
          :class="['table-tag', { 'is-template': row.isTemplate }]"
        >
          {{ row.isTemplate ? $t('模版') : $t('实例') }}
        </span>
        <div
          v-if="row.isTemplate"
          @click="handleEidtDisabled(row)"
          class="text-cursor"
        >
...
export default {
  ...
  methods: {
    ...
    // 需要把这个fn做成参数传递过去
    handleCellEdit(data) {
      $emit(this, 'cell-edit', data)
    },
    handFilterUndone(data) {
      $emit(this, 'sub-head-filter', data)
    },
    handleEidtDisabled(row) {
      $emit(this, 'eidt-disabled', row)
    },
    handleCancelSelect() {
    ...
    getellClassName({ row, rowIndex, columnIndex }) {
      const nameStr =
        !rowIndex &&
        this.tableHead[columnIndex] &&
        this.tableHead[columnIndex].isExpandable
          ? 'expand-th'
          : ''
      return this.isBizProcess &&
        row[columnIndex].property === 'bk_process_name'
        ? `${nameStr} show-tag-column`
        : nameStr
    },
    setCellClassName({ row, column }) {
      return column.property === 'bk_process_name' && row.showTypeTag
        ? 'show-tag-column'
        : ''
    },
    // 表格无数据 - 只缺实例或者进程
