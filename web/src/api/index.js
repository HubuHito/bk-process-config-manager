...
axiosInstance.interceptors.request.use(
  (config) => {
    ...
    if (query[window.AJAX_MOCK_PARAM]) {
    ...
    } else if (window.PROJECT_CONFIG.IS_MOCK) {
      // 后端接口返回 MOCK 数据，可避免权限问题，方便体验静态页面
...
function handleReject(error, config) {
  const { messageError } = window.$vueApp.config.globalProperties
...
export function injectCSRFTokenToHeaders() {
  const CSRFToken = cookie.parse(document.cookie)[
    window.PROJECT_CONFIG.CSRF_COOKIE_NAME
  ]
